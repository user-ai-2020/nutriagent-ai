...
export function DashboardClient() {
  ...
  return (
    ...
    <div style={{ width: "100%" }}>
      ...
      <div style={{ display: "grid", gap: "var(--space-4)", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", marginBottom: "var(--space-6)" }}>
        <WeeklyCalorieTrend data={data.dailyBreakdown} />
        <MacroBreakdownChart data={[
          { name: "Protein", value: data.todayTotals.protein },
          { name: "Carbs", value: data.todayTotals.carbs },
          { name: "Fat", value: data.todayTotals.fat },
          { name: "Sugar", value: data.todayTotals.sugar }
        ]} />
        <StepsVsCaloriesChart data={data.dailyBreakdown} />
      </div>
    </div>
  );
}
